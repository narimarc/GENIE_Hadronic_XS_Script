  ...
  //Find file type
      ...
      if(gType.compare("XS")==0){
        ...
        if(pos != string::npos){
          if(filename[pos+1] == 'r'){
            if(filename[pos-1] == 'e'){
              if(filename[pos-8]=='L') fileAuthor.assign("HINCL");
              else if(filename[pos-8]=='c') fileAuthor.assign("hg4bertcasc");
              else fileAuthor.assign("NULL");
              type.assign("ginuke");
            }
            else{
              type.assign("gst");
        ...
        //Normal .ginuke file, Easy
            ...
            if(file->IsOpen()){
              TTree* dummy = (TTree*)file->Get(type.c_str());
...
//  when asked nicely.
    ...
    public:
      ...
      DataFile* makeDataFile(int which,string dir,string source);
      ...
      if(line[0]!='#'){ //Ignore lines starting with a hash
        ...
        if(curTag.compare("PROCESS")==0){
          if(depth==1){
            ...
            //cout<<" Here we are in the file "<<line<<"\n";
...
DataFile* FormatFile::makeDataFile(int which,string dir,string source){
  ...
  if(this->type.compare("Angle")==0){
     temp = new DataFile(type,dir,fileNames[i], titles[i],cols[i],cuts[i],1.0,i+1);
  }
  else if(this->type.compare("Momentum")==0||this->type.compare("XS")==0){
     temp = new DataFile(type,dir,fileNames[i], titles[i],cols[i],cuts[i],dcths[i],i+1);
  }
  else{
     temp = new DataFile(type,dir,fileNames[i], titles[i],cols[i],cuts[i],dcths[i],i+1);
  }
...
int rootgINukeVal(char* fFile, char* dataDir = ".", char* ROOTDir = ".",char* saveDir=".")
{
 ...
 while(doCurrent == true){
  ...
  cans->cd();
  //TPad* curP = gPad;
  if(format.logx==true){gPad->SetLogx();} // Remove (1) -> ()
  if(format.logy==true){gPad->SetLogy();} // Remove (1) -> () Jun_19_19
  TextSize = .035;
    ...
    hf1->Draw();
           // hf1 ->GetYaxis()->SetRangeUser(0,1); ///   ETO No tena izy d mba esory avy eo O
    ...
    for(k=0;k<numRoots;k++){
      ...
      if(format.type.compare("XS")==0){
        ...
        cout<<" This is SimData->cut_c_str() "<<simData->cut.c_str()<<endl;
        ...
        if(simData->valid()){
          ...
          htemp = (TH1F*) gPad->GetPrimitive("htemp");
                    //   htemp->GetYaxis()->SetRangeUser(0,1);
          TH1F* hist1;
          TH1F* hist1error;
          good=false;
          ...
          if (htemp != 0x0) {
            ...
            simData->dataTree->SetBranchAddress("probe",&pdgcProbe);
            simData->dataTree->GetEntry(1);
            A = (target/10) - (target/10000)*1000;
            int Z = (target/10000)-(target/1000000)*100; 
            factor = TMath::Power(3*1.4*TMath::Power(A,(1./3.)),2)*10.0/(2*num*simData->dcth);
            
            //----------------For INCL++------------------------------//
            double EKin,ek,epsilon(0.001), pp, energyp;
            simData->dataTree->SetBranchAddress("ke",&ek);
            simData->dataTree->SetBranchAddress("e",&energyp);
            simData->dataTree->SetBranchAddress("p",&pp);
            simData->dataTree->GetEntry(1);

            EKin= ek*1000;
            pp = pp*1000;
            energyp =energyp*1000;
            const double pi = 3.14159265358979323846264338328;
            const double tenPi = 10.0 * pi;
            TLorentzVector probe(.0,.0,pp,energyp); 
            std::string ParticleSpecies;
     
              factor =TMath::Power(3*1.4*TMath::Power(A,(1./3.)),2)*10/(2*num*simData->dcth);

              // ****************************Compute geometric cross section for INCL++
              
              if (fileAuthor=="HINCL") {
                if (pdgcProbe==2212) ParticleSpecies="Proton";
                else if (pdgcProbe==2112) ParticleSpecies="Neutron";
                else if (pdgcProbe==211) ParticleSpecies="PiPlus";
                else if (pdgcProbe==-211) ParticleSpecies="PiMinus";
                else if (pdgcProbe==111) ParticleSpecies="PiZero";
                double geomXSINCL= maxImpactParameter(ParticleSpecies, probe,A, Z);
                               factor = (10*geomXSINCL*geomXSINCL*TMath::Pi())/(2*TMath::Pi()*num*simData->dcth);
    
              }
             else if(fileAuthor=="hg4bertcasc"){
                double C1=3.3836;
                double C2=1.7234;               
                C1=C1*TMath::Power(A,(1./3.));
                double rMax = C2*TMath::Log((1+TMath::Exp(-C1/C2))/0.01-1)+C1;
                factor=10*(rMax*rMax*TMath::Pi())/(2*TMath::Pi()*2*num*simData->dcth);

              }
              else factor =TMath::Power(3*1.4*TMath::Power(A,(1./3.)),2)*10.0/(2*num*simData->dcth);
	    ...
	    //make and draw an extra histogram to have solid error bars
            ...
            hist1error->GetYaxis()->SetRangeUser(0,1);
          }
        }
        else{
   ...
   for(int j=0;j<numFiles;j++){
      ...
      leg1->AddEntry(data1,experimental->title.c_str(),"P");
      data1->Draw("p same");
      data1->GetYaxis()->SetRangeUser(0,1);
    }
  /*
   //***************************************

  for(int j=0;j<numFiles;j++){
    DataFile* experimental = format.makeDataFile(j,"Nuwro","EXPERIMENTAL"); 
 
    TGraphErrors* data1;
    experimental->dataTree->Draw(experimental->cols.c_str(),"","goff");
    data1 = new TGraphErrors(experimental->dataTree->GetSelectedRows(),experimental->dataTree->GetV2(), experimental->dataTree->GetV1(),0,experimental->dataTree->GetV3());
      data1->SetLineStyle(3);
      data1->SetMarkerColor(kYellow); //kYellow kGreen
  
      data1->SetMarkerStyle(markerStyle);
      markerStyle++;
      leg1->AddEntry(data1,"Nuwro","P");
      data1->Draw("p same");
    }
   //**************************************** 
    */
